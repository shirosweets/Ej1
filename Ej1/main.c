...
bool esCima(int a[], int length){ // Inicio funcion esCima
  // cima es el valor max del arreglo
  int cima=0; // [1, 2, 3, 4, 3, 2, 1] length=7
  bool cima_res = true;
  if(length<=0){
      printf("\n El arreglo NO puede tener cima porque length<=0 \n\n");
      cima_res=false;
  }
  for(int i=1; i<length && (cima_res==true); i++){
    if(a[cima]<a[i]){ //a[0]<a[1]  -> 1<2 cima=1 -> a[1]<a[2] -> 2<3 cima=2 -> a[2]<a[3] -> 3<4 cima=3 -> a[3]<a[4] -> 4>3 //
      //printf("\n El arreglo puede tener cima\n ");
      cima=i;
    }
    else{
      if(a[cima]>a[i] && a[i]>a[i+1]){ // a[3]>a[4] -> 4>3 && a[4]>a[5]
        // cima queda exactamente igual
        //printf("\n El arreglo puede tener cima (else, if)\n ");
      }
      else{
        //printf("\n El arreglo NO puede tener cima\n ");
        cima_res=false;
        printf("\n\n >>>> El arreglo NO tiene cima <<<<<\n\n");
      }
    }
  }
    if (cima_res==true){
        printf("\n\n >>>>  El arreglo tiene cima <<<<<\n\n");
    }
    /*else{
        printf("\n\n >>>> El arreglo NO tiene cima <<<<<");
    }*/
